...
class LessonsView(APIView):
    """Minimal endpoint used to demonstrate role-based lesson access."""

    permission_classes = (IsTeacherOrReadOnly,)

    def get(self, request):
        return Response({'detail': 'Lessons are available.'})

    def post(self, request):
        return Response({'detail': 'Lesson created.'}, status=status.HTTP_201_CREATED)
