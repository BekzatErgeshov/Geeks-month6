...
class AuthenticationAPITests(APITestCase):
    credentials = {
        ...
        'phone': '+996700000000', 'role': CustomUser.Role.STUDENT,
...
class GoogleOAuthAPITests(APITestCase):
    ...
    def test_google_authorization_creates_then_reuses_user(self, get_access_token, get_user_info):
        ...
        self.assertEqual(first.data['user']['role'], CustomUser.Role.STUDENT)
...
class LessonPermissionTests(APITestCase):
    def setUp(self):
        self.student = CustomUser.objects.create_user(
            email='student@example.com', password='secure-password-123'
        )
        self.teacher = CustomUser.objects.create_user(
            email='teacher@example.com', password='secure-password-123',
            role=CustomUser.Role.TEACHER,
        )

    def test_authenticated_student_can_read_but_cannot_create(self):
        self.client.force_authenticate(self.student)
        self.assertEqual(self.client.get('/api/v1/lessons/').status_code, status.HTTP_200_OK)
        self.assertEqual(self.client.post('/api/v1/lessons/', {}, format='json').status_code, status.HTTP_403_FORBIDDEN)

    def test_teacher_can_create(self):
        self.client.force_authenticate(self.teacher)
        self.assertEqual(self.client.post('/api/v1/lessons/', {}, format='json').status_code, status.HTTP_201_CREATED)

    def test_anonymous_user_cannot_read_lessons(self):
        self.assertIn(self.client.get('/api/v1/lessons/').status_code, (status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN))
